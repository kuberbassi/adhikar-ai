...
router.post('/', async (req, res) => {
    try {
        ...
        if (aiResult) {
            console.log(`🤖 AI Analysis: ${aiResult.violation} (${aiResult.act}) [Case: ${caseId}] in ${analysisDuration.toFixed(1)}s`);

            // Save to database for legal protection (non-blocking)
            saveCaseRecord(caseId, {
                ip,
                ...
                status: 'analyzed',
                source: 'ai'
            }).catch(e => console.error('Background save failed:', e.message));
        ...
        console.error('❌ Case analysis failed:', err.message);
        logActivity('analysis_error', { message: err.message, path: '/api/analyze' }).catch(() => { });
        res.status(500).json({
            error: 'Analysis failed',
            message: 'Something went wrong during the legal analysis. Please try again or consult a professional.'
        });
    }
});
